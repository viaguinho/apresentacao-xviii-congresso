...
const MOTIQ_TOKENS = "@layer motiq{:root{--motiq-accent:#0071e3;--motiq-accent-text:#0071e3;--motiq-bg:#f7f9fc;--motiq-border:#dce4ef;--motiq-border-strong:#c5d1e1;--motiq-fg:#101828;--motiq-fg-secondary:#344054;--motiq-muted:#667085;--motiq-secondary-accent:#0ea5e9;--motiq-signature:#6366f1;--motiq-surface:#ffffff;--motiq-surface-2:#f8fafd}}@layer motiq{.dark,[data-theme=\"dark\"]{--motiq-accent:#0071e3;--motiq-accent-text:#38bdf8;--motiq-bg:#080c14;--motiq-border:#263449;--motiq-border-strong:#354863;--motiq-fg:#f8fafc;--motiq-fg-secondary:#cbd5e1;--motiq-muted:#9caabd;--motiq-secondary-accent:#22c7d9;--motiq-signature:#818cf8;--motiq-surface:#111827;--motiq-surface-2:#192337}}";
 ...
 */
export function comet(tail: string, head: string, tip: string, midAlpha: number, start: number): string {
  return [
...
export function ringGradient(beams: 1 | 2, colors: [string, string?] | undefined): string {
  const tail0 = colors?.[0] ?? "var(--motiq-accent, #0071e3)";
  const head0 = colors?.[0] ?? "var(--motiq-secondary-accent, #0ea5e9)";
  const stops = [
    "transparent 0deg",
    comet(tail0, head0, `color-mix(in srgb, ${head0} 22%, #ffffff)`, 55, 0),
  ];
  if (beams === 2) {
    const c1 = colors?.[1] ?? "var(--motiq-signature, #6366f1)";
...
export function BorderBeamPanelBase({
  children,
  beams = 2,
  colors,
  thickness = 2,
...
@media (forced-colors: active) {
  ...
  return (
    <div
      ...
      onBlur={settle}
      className={cn(
        "relative w-full border border-black/[0.08] bg-white p-7",
        cls,
        className,
      )}
      style={{

...
export function PatternLines({
  ...
  width = 6,
  height = 6,
  stroke = "var(--chart-line-primary, #0071e3)",
  strokeWidth = 1,
...
export interface FunnelChartProps {
  ...
  labelLayout?: "spread" | "grouped" | "custom";
  labelOrientation?: "vertical" | "horizontal";
  labelAlign?: "center" | "start" | "end";
  renderCustomLabel?: (stage: FunnelStage, index: number, isHovered: boolean) => ReactNode;
  grid?:
    | boolean
    | {
        bands?: boolean;
...
function vSegmentPath(
  ...
  const mx = W / 2;
  const w0 = Math.max(normStart * W * 0.46 * layerScale, 12);
  const w1 = Math.max(normEnd * W * 0.46 * layerScale, 12);
...
function HRing({
  ...
  const extraScale = 1 + (ringIndex / Math.max(totalRings - 1, 1)) * 0.08;
  const ringSpring = {
...
function VRing({
  ...
  const extraScale = 1 + (ringIndex / Math.max(totalRings - 1, 1)) * 0.06;
  const ringSpring = {
    stiffness: 300 - ringIndex * 60,
    damping: 24 - ringIndex * 3,
  };
...
function HSegment({
  ...
  const rings = Array.from({ length: layers }, (_, l) => {
    ...
    const opacity = 0.22 + (l / (layers - 1 || 1)) * 0.68;
    return {
...
function SegmentLabel({
  ...
  const valueEl = showValues && (
    <span className="whitespace-nowrap font-bold text-foreground text-sm tracking-wide">
      {display}
    </span>
  );
  const pctEl = showPercentage && (
    <span className="rounded-full bg-white/90 px-3 py-1 font-bold text-[#0071e3] text-xs shadow-sm border border-[#0071e3]/20">
      {formatPercentage(pct)}
    </span>
  );
  const labelEl = showLabels && (
    <span className="whitespace-nowrap font-semibold text-slate-800 text-sm uppercase tracking-wider">
      {stage.label}
    </span>
  );
  ...
  if (layout === "spread") {
    return (
      ...
      >
        {isHorizontal ? (
          <>
            <div className="flex h-[18%] items-end justify-center pb-1">
              {valueEl}
            </div>
            <div className="flex flex-1 items-center justify-center">
              {pctEl}
            </div>
            <div className="flex h-[18%] items-start justify-center pt-1">
              {labelEl}
            </div>
          </>
        ) : (
          <>
            <div className="flex w-[22%] items-center justify-end pr-3">
              {labelEl}
            </div>
            <div className="flex flex-1 items-center justify-center">
              {pctEl}
            </div>
            <div className="flex w-[22%] items-center justify-start pl-3">
              {valueEl}
            </div>
          </>
        )}
...
export function FunnelChart({
  ...
  const bandColor = gridCfg.bandColor ?? "rgba(0,113,227,0.04)";
  const showGridLines = gridEnabled && (gridCfg.lines ?? true);
  const gridLineColor = gridCfg.lineColor ?? "rgba(0,113,227,0.15)";
  ...
  return (
    <div
      ...
      ref={ref}
      style={{
        aspectRatio: horiz ? "2.2 / 1" : "1 / 1.15",
        ...style,
      }}
    >
      {W > 0 && H > 0 && (
        <>
          ...
          >
            {data.map((stage, i) => {
              ...
              if (horiz) {
                ...
                // Ascending: Top (index 0) starts narrow and widens downwards to base
                normStart = norms[i] ?? 0;
                normEnd = norms[Math.min(i + 1, n - 1)] ?? 0;
              } else {
              ...
              return horiz ? (
                ...
                <VSegment
                  ...
                  staggerDelay={staggerDelay}
                  straight={edges === "straight"}
                  transformOrigin={isAscending ? "center bottom" : "center top"}
                />
              );
            })}
          </div>
          ...
          {/* Label overlays / Interactive click & hover triggers */}
          {data.map((stage, i) => {
            ...
            return (
              ...
              >
                {renderCustomLabel ? (
                  renderCustomLabel(stage, i, isHovered)
                ) : (
                  <SegmentLabel
                    align={labelAlign}
                    ...
                    index={i}
                    isHorizontal={horiz}
                    layout={labelLayout === "custom" ? "spread" : labelLayout}
